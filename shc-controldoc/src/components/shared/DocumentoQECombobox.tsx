...
import { useQualityEvents } from '../../features/quality-events/hooks/useQualityEvents'

interface DocumentoQEComboboxProps {
  mode: 'document-search-qe' | 'qe-search-document'
  linkedIds: string[]
...
export function DocumentoQECombobox({ mode, linkedIds, onSelect, ariaLabel }: DocumentoQEComboboxProps) {
  const { t } = useTranslation(mode === 'document-search-qe' ? 'documents' : 'qualityEvents')
  ...
  const qeQuery = useQualityEvents(
    { search: debouncedQuery, page: 1, pageSize: 10 },
    searchActive && mode === 'document-search-qe',
  )
  const documentsQuery = useDocuments(
    { search: debouncedQuery, pageSize: 10 },
    searchActive && mode === 'qe-search-document',
  )

  const isSearching = mode === 'document-search-qe' ? qeQuery.isFetching : documentsQuery.isFetching

  const results: ResultOption[] =
    mode === 'document-search-qe'
      ? (qeQuery.data?.items ?? [])
          .filter((qe) => !linkedIds.includes(qe.id))
          .map((qe) => ({ id: qe.id, primary: qe.numero, secondary: `${qe.tipo} · ${qe.severidad} · ${qe.estado}` }))
  ...
  const placeholder =
    mode === 'document-search-qe'
      ? t('qeVinculados.combobox.placeholder')
