...
interface ConfirmarDesactivarAreaModalProps {
  nombre: string
  isPending: boolean
  onConfirm: () => void
  onClose: () => void
...
export function ConfirmarDesactivarAreaModal({
  nombre,
  isPending,
  onConfirm,
  onClose,
}: ConfirmarDesactivarAreaModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 dark:bg-black/60">
      <div className="relative w-full max-w-md rounded-xl bg-canvas p-6 shadow-xl dark:bg-surface-dark-elevated">
        ...
        <div className="mb-4 flex items-start gap-3">
          ...
          <div>
            <h2 className="font-medium text-ink dark:text-on-dark">{t('confirmarDesactivar.titulo')}</h2>
            <p className="mt-1 text-sm text-muted dark:text-on-dark-soft">
              {t('confirmarDesactivar.mensaje', { nombre })}
            </p>
          </div>
        </div>
