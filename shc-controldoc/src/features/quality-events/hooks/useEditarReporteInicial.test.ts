...
describe('PATCH /api/quality-events/:id/editar-reporte-inicial', () => {
  it('returns 404 for an unknown id', async () => {
    ...
    result.current.mutate({ id: 'does-not-exist', data: { areaId: 'Almacén' } })
  ...
  it('updates only changed fields and appends one audit entry per field', async () => {
    ...
    expect(result.current.data?.areaId).toBe('Almacén Sur')
    ...
    expect(nuevasEntradas?.[0].campoModificado).toBe('areaId')
  })
})
