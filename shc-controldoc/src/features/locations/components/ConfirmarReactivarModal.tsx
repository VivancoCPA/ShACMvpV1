...
interface ConfirmarReactivarModalProps {
  tipo: 'local' | 'zona'
  nombre: string
  isPending: boolean
  onConfirm: () => void
...
export function ConfirmarReactivarModal({
  tipo,
  nombre,
  isPending,
  onConfirm,
  onClose,
}: ConfirmarReactivarModalProps) {
  const { t } = useTranslation('locations')
  const titulo =
    tipo === 'local' ? t('confirmarReactivar.tituloLocal') : t('confirmarReactivar.tituloZona')
  const mensaje =
    tipo === 'local'
      ? t('confirmarReactivar.mensajeLocal', { nombre })
      : t('confirmarReactivar.mensajeZona', { nombre })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 dark:bg-black/60">
      <div className="relative w-full max-w-md rounded-xl bg-canvas p-6 shadow-xl dark:bg-surface-dark-elevated">
        ...
        <div className="mb-4 flex items-start gap-3">
          <RotateCcw size={20} className="mt-0.5 shrink-0 text-teal" />
          <div>
