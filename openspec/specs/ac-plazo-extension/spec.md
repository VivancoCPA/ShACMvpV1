# Spec: ac-plazo-extension

## Purpose

AC deadline-extension ("solicitud de ajuste de plazo") flow for QE-owned `AccionCorrectivaQE`. Lets the AC's responsable propose a new `plazoFecha` with a justification, routes approval to either `JEFE_CALIDAD_SYST` or `ALTA_DIRECCION` based on severity/increment thresholds, and preserves full request history per AC.
