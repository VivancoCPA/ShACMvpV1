...
CRUD administrativo de usuarios en el backend real (`ShcMvpEndPoint`), scoped por empresa activa del actor: listado (con excepción para `SUPERADMIN`, que ve todos los usuarios sin filtro de empresa), alta, edición (con rol aislado por empresa vía `UsuarioEmpresa`), baja/reactivación sin condición de bloqueo, y reset de contraseña por administrador. Implementa server-side las reglas de negocio RN-USR-001 a RN-USR-007 ya establecidas por el mock del frontend (`user-management-msw`). Todas las mutaciones y el listado (salvo la excepción de `SUPERADMIN`) están restringidos al rol `ADMINISTRADOR_EMPRESA`.
