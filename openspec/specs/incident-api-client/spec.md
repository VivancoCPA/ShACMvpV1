# Spec: incident-api-client

## Purpose

Typed Axios-based API client for the Incidents module (`src/api/endpoints/incidents.api.ts`). Provides one function per REST endpoint so that TanStack Query hooks never call Axios directly. All functions reuse the shared `api` instance from `src/lib/axios.ts`.
