# Spec: dashboard-altadireccion-view

## Purpose

Define the `ALTA_DIRECCION` role's executive dashboard: the `AltaDireccionDashboard` page and its 7 widgets in fixed order (`AccionesRequeridasWidget`, `KpisEjecutivosWidget`, `ComparativaMensualWidget`, `QEsCriticosWidget`, `ReaperturasWidget`, `ACsExtensionPlazoWidget`, `HeatmapIncidentesWidget`), consuming `useDashboardSummary()` for organization-wide KPIs, monthly trend comparisons, critical/reopened Quality Events, pending plazo-adjustment requests on ALTA/CRITICA QEs, and the shared incident heatmap by local.
