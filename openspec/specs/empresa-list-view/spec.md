# Spec: empresa-list-view

## Purpose

Define `EmpresasAdminPage` (route `/admin/empresas`) and its `EmpresaList` table: the SUPERADMIN-only screen listing every empresa in the system, with activate/deactivate actions that carry explicit impact warnings (RN-EMP-005), and a per-row link into the per-empresa user assignment screen.
