# frontend-incidentes-cutover-verification

## Purpose

Verificación manual en navegador del CRUD de Incidentes SyST (M3) de `shc-controldoc` contra el backend .NET real + Postgres real, con `VITE_ENABLE_MSW=false` — sin tocar UI ni lógica de cliente. Cubre creación, listado, detalle, actualización de investigación (incluidos los campos mobile: testigos, personal involucrado, equipos involucrados, atención médica, notificación ambiental), cambio de estado, eliminación (soft-delete) y restauración de Incidentes; las Acciones Correctivas del Incidente incluido el nuevo endpoint de cierre; la vinculación Incidente↔Quality Event; el ciclo completo de reporte offline de la PWA mobile (encolado, Background Sync, subida real de evidencias y geolocalización) con Service Worker activo; y el aislamiento multi-tenant del `empresaId` capturado al encolar un reporte offline. Documenta explícitamente qué escenarios se verificaron por API directa (sin punto de entrada en la UI actual) frente a los verificados de punta a punta en el navegador. Equivalente de Incidentes a `frontend-catalogos-cutover-verification` (change `cutover-catalogos`).
