...
When the second signature transitions a QE to `CERRADO`, the client SHALL show a Sonner success toast to the signing user. If `qe.severidad` is `'ALTA'` or `'CRITICA'` (per `requiereNotificacionUrgente`-style severity check), an additional toast SHALL indicate Gerencia has been notified; a toast SHALL also indicate the original reporter (`qe.reportadoPorId`) has been notified, in both cases as an in-app mock notification (no real email/push).

#### Scenario: Standard closure toast
- **WHEN** the second signature succeeds for a QE with `severidad: 'MEDIA'`
- **THEN** a success toast confirming closure is shown, with no additional Gerencia toast

#### Scenario: CRITICA closure notifies Gerencia
- **WHEN** the second signature succeeds for a QE with `severidad: 'CRITICA'`
- **THEN** an additional toast indicating Gerencia has been notified is shown
