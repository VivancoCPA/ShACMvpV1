...
Ruta mobile-first para reporte rápido de incidentes SyST (`/m/incidentes/nuevo`), con un subset de campos del formulario de escritorio, captura de foto y GPS opcionales, y envío contra el mismo endpoint mock `POST /api/incidents` usado por el flujo de escritorio, con cola offline local ante falta de conexión.
