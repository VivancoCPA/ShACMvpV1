...
Axios API client and TanStack Query v5 hooks for M6 user administration (list, create, update, toggle-active, reset-password), following the component → hook → `*.api.ts` → MSW layering convention. Mutations invalidate `QUERY_KEYS.users.all` and record `AuditTrailEntry` entries.
