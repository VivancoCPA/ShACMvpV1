# be-no-conformidades-api

## Purpose

Backend .NET del módulo de No Conformidades (M2): creación con cálculo server-side de `requiereIPER` (RN-NC-001), numeración correlativa por empresa y dominio, detección de posibles duplicados (RN-NC-005), notificación best-effort a Comercio Exterior para NCs de dominio `ADUANERO` (RN-NC-002), listado paginado y filtrado, edición parcial bloqueada en estados terminales, anulación, eliminación (soft-delete) y restauración restringidas al estado `ABIERTA`, gestión de acciones correctivas de la NC (creación, actualización con registro de extensión de plazo sobre AC vencida — RN-NC-006 —, y cierre con evidencia), y aislamiento multi-tenant estricto scoped por `empresaId` de la sesión activa.
