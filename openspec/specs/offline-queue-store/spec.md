...
Store de Zustand (`offlineQueueStore`) que proyecta el estado de la cola offline de incidentes (`incident-queue` en IndexedDB, ver `offlineQueue.ts`) para consumo de la UI mobile — sin persistencia propia, siempre recalculado desde IndexedDB como única fuente de verdad. Alimenta el badge/panel de `offline-queue-indicator` y expone `retry(localId)` delegando en la cola offline subyacente.
