# Spec: user-profile-change-password

## Purpose

Formulario de cambio de contraseña de login propia dentro de `/perfil` (M6-S06). Cubre únicamente la contraseña de `authStore`/`auth.fixtures.ts`; el PIN de firma electrónica de aprobación de documentos (RN-DOC-004) queda fuera de alcance por ser un mecanismo distinto y más crítico ligado a la firma de documentos.
