...
Define the authentication pages (Login, ForgotPassword, ResetPassword), the dev-mode role selector, the password strength indicator, shared password validation rules, the empresa-selection step during login, and the MSW handlers for all auth endpoints.
