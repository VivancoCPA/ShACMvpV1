# Spec: notification-generation

## Purpose

Pure helper functions in `src/mocks/fixtures/notificationGeneration.ts` used by MSW handlers across domains to create notifications for state changes (RN-NOTIF-001), assignments (RN-NOTIF-002), and idempotent deadline/vencimiento scans (RN-NOTIF-003), plus the identity guard that determines whether an id has a real, loggable account.
