# frontend-catalogos-cutover-verification

## Purpose

Verificación manual en navegador de los 4 catálogos (Area, Empresa, Local, Zona) de `shc-controldoc` contra el backend .NET real + Postgres real, con `VITE_ENABLE_MSW=false` — sin tocar UI ni lógica de cliente, ya que el contrato ya coincide con MSW. Cubre CRUD completo de Area/Local/Zona, multipart de Local (plano PNG), el detalle de Local con Zonas embebidas y el listado de Zonas sin filtro (ambos dependientes de los dos endpoints backend nuevos de `be-locales-zonas-lectura`), el CRUD de Empresa y gestión de usuarios de empresa gateado a `SUPERADMIN`, y el aislamiento multi-tenant de los 4 catálogos. Equivalente de catálogos a `frontend-auth-cutover-verification` (change `cutover-auth`).
