# Spec: empresa-admin-permissions

## Purpose

Define the render-time permission helpers gating the empresa-administration module — who may administer the `Empresa` catalog and who may assign or revoke users across empresas — plus the RN-EMP-005 cascade that deactivating an `Empresa` triggers on its `UsuarioEmpresa` rows. This module is exclusive to the `SUPERADMIN` role.
