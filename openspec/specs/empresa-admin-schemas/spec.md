# Spec: empresa-admin-schemas

## Purpose

Define the Zod validation schemas backing the empresa-administration module: `empresaFormSchema` for empresa create/edit (razón social, RUC, estado, logo), its RUC-uniqueness rule enforced server-side in the mock handler, and `asignarUsuarioEmpresaSchema` for assigning an existing user to an empresa with a role (excluding `SUPERADMIN`).
