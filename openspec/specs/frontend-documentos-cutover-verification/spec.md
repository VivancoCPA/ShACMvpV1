# frontend-documentos-cutover-verification

## Purpose

Verificación manual en navegador del ciclo de vida completo de Documentos (M1 Control Documentario) de `shc-controldoc` contra el backend .NET real + Postgres real, con `VITE_ENABLE_MSW=false` — sin tocar UI ni lógica de cliente salvo el cambio de contrato de firma (`password`/`timestamp` → `pin`) cubierto en `document-approval-flow`. Cubre creación, listado, detalle, edición y las 6 transiciones de la máquina de estados de Documentos; la verificación de RN-DOC-001 (obsoletización automática) y RN-DOC-005 (bloqueo por QE vinculado activo) en un flujo de firma real disparado desde `DocumentSignatureModal`; la gestión de archivos (original editable, distribución, PDF controlado) contra el backend real; la confirmación de que la vinculación Documento↔QE y Documento↔NC cutover-eada en sesiones anteriores sigue funcionando; y el reversado de `VITE_ENABLE_MSW` a `true` al cerrar la verificación para no bloquear el desarrollo de los módulos aún no cutover-eados. Equivalente de Documentos a `frontend-catalogos-cutover-verification` (change `cutover-catalogos`) y `frontend-incidentes-cutover-verification` (change `cutover-incidentes`).
