# Spec: empresa-usuario-types

## Purpose

TypeScript type definitions for the `UsuarioEmpresa` join entity, which associates a `User` with an `Empresa` and carries a per-company `rol`. All types are defined in `src/features/empresas/types/empresa.types.ts`. (Purpose is brief/TBD pending broader multi-empresa design docs beyond this phase.)
