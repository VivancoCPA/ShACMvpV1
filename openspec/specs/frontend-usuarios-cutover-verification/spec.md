# frontend-usuarios-cutover-verification

## Purpose

Verificación manual / API directa del ciclo de vida completo de Usuarios (M6) de `shc-controldoc` contra el backend .NET real + Postgres real, con `VITE_ENABLE_MSW=false` — sin tocar UI ni lógica de cliente, ya que la investigación previa no encontró ningún mismatch de contrato entre frontend y backend. No hay automatización de navegador disponible, por lo que la verificación se realiza mediante llamadas directas a la API con un JWT real. Cubre el listado con filtros `rol`/`activo` (incluido el guard de ruta que restringe `/usuarios` a `ADMINISTRADOR_EMPRESA`), la creación de usuario (validación de avatar espejada en ambos lados y contraseña temporal real), la edición (incluida la confirmación de que un cambio de `rol` queda acotado a la fila `UsuarioEmpresa` de la empresa activa del actor y no se filtra a otras empresas del mismo usuario), las dos variantes del conflicto de email duplicado (misma empresa vs. otra empresa), la baja/reactivación y el reseteo de contraseña (ambos sin validación de bloqueo), la excepción `SUPERADMIN` de `GET /api/users` (verificada por API directa, sin punto de entrada en la UI) y la reversión de MSW a activo al cerrar. Equivalente de Usuarios a `frontend-no-conformidades-cutover-verification` y `frontend-quality-events-cutover-verification`.
