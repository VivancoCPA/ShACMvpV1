...
Herramienta dev-only que reinicia los stores mock de MSW (Documentos, Quality Events, No Conformidades, Incidentes, Locales/Zonas) a su estado de fixtures original sin necesidad de reiniciar el servidor de desarrollo, evitando falsas alarmas de QA por mutaciones que persisten entre sesiones de prueba manual dentro de la misma pestaña. Vive en `src/pages/dev/DevResetMocksPage.tsx` (ruta `/dev/reset-mocks`) y `src/mocks/resetAllStores.ts`.
...
- **THEN** el usuario permanece autenticado y no es redirigido a `/login` (la sesión persiste vía `mockSession.ts` en `localStorage`, mecanismo independiente de este reset)
