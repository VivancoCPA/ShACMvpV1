...
TanStack Query v5 hooks for the M1 Document module. All hooks live in `src/features/documents/hooks/useDocuments.ts`, call only functions from the API client (`src/api/endpoints/documents.api.ts`), and follow the SHAC query-key convention (`QUERY_KEYS.documents.*`).
