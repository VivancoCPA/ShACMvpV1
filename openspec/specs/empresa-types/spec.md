...
TypeScript type definitions for the `Empresa` (company) entity — the multi-company catalog that anchors the first phase of the multi-empresa data model. All types are defined in `src/features/empresas/types/empresa.types.ts`. (Purpose is brief/TBD pending broader multi-empresa design docs beyond this phase.)
