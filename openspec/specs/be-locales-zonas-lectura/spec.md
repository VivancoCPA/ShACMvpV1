...
Backend .NET de Locales y Zonas, scoped por `empresaId` de la sesión activa: lectura (`GET /api/locales`, `GET /api/locales/:localId/zonas`) y CRUD administrativo completo (creación con límite de cupo activo y plano PNG, edición, desactivación con validación de Incidentes, reactivación, y sus equivalentes para Zona), más el seed de datos de desarrollo. Las mutaciones están restringidas a `ADMINISTRADOR_SISTEMA` y `JEFE_CALIDAD_SYST`.
