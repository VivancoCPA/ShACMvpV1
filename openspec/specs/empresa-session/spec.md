# Spec: empresa-session

## Purpose

Define resolution and persistence of the active empresa in the session (login, refresh, in-session switch), computation of the effective role from `UsuarioEmpresa`, and the consolidation of domain MSW handlers around a single session-derived acting user.
