...
Backend .NET del catálogo de Áreas (`AREAS_SHAC`), sembrado por empresa en entorno de desarrollo y expuesto en modo de solo lectura (`GET /api/areas`, `GET /api/areas/:id`), scoped por `empresaId` de la sesión activa igual que Locales/Zonas. No incluye mutaciones (creación/edición/desactivación) en este alcance.
