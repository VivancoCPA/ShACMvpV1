# Spec: notifications-realtime-hub

## Purpose

SignalR hub (`.NET` backend) that pushes `SEVERIDAD_CRITICA`/`CIERRE` notifications to connected clients in real time, grouped by user and scoped by company, after the corresponding `Notificacion` row has been persisted.
