...
Document detail page for M1 Control Documentario. Renders `/documentos/:id` with a two-column layout: header + tabs (Detalle, Historial, Audit Trail) on the left and a sticky action panel on the right. Implements contextual banners, confidencialidad badges, version history timeline, and audit trail pagination.
