...
El sistema SHALL responder KPI-06 con `valor: 0` (semáforo calculado igual que cualquier otro KPI absoluto contra su meta de 100), documentando en el código que el valor es `0` porque el módulo de Documentos no existe todavía en este backend, no porque no haya documentos vigentes.

#### Scenario: Solicitud de KPI-06
- **WHEN** se solicita `GET /api/dashboard/kpis`
- **THEN** el `KpiResult` de `kpiId: 'KPI-06'` responde `valor: 0` y `semaforo: 'ROJO'`
