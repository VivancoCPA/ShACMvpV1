...
### Requirement: El código de sesión mock-only queda retirado una vez verificado que el backend real no lo necesita

El sistema SHALL eliminar los headers `X-Mock-Refresh-Token` y `X-Mock-Empresa-Activa` enviados por `authStore.ts` en cada llamada a `/api/auth/refresh`, y las funciones `persistMockRefreshToken`/`readMockRefreshToken` de `lib/mockSession.ts`, una vez confirmado mediante los escenarios de este documento que el backend real resuelve `empresaActivaId` y el token de refresh exclusivamente a través de la cookie `httpOnly` y la tabla `RefreshTokens`, sin leer ningún header adicional.

#### Scenario: Refresh funciona sin enviar headers mock-only

- **WHEN** `authStore.refreshToken()`/`authStore.bootstrap()` llaman a `POST /api/auth/refresh` sin los headers `X-Mock-Refresh-Token`/`X-Mock-Empresa-Activa`
- **THEN** el backend real responde 200 con la sesión correcta, apoyándose únicamente en la cookie `shac_refresh_token`

#### Scenario: `persistActiveEmpresaId`/`readActiveEmpresaId` se mantienen para la sugerencia de UI en login

- **WHEN** se inspecciona `lib/mockSession.ts` tras la limpieza
- **THEN** `persistActiveEmpresaId`/`readActiveEmpresaId` siguen presentes (usados por `LoginPage.tsx` para preseleccionar la empresa de la última sesión), mientras que `persistMockRefreshToken`/`readMockRefreshToken` fueron eliminados
