...
The system SHALL render a `QEListFilters` component that manages `estado`, `tipo`, `severidad`, `origen`, `areaAfectada`, `fechaDesde`, `fechaHasta`, `soloReincidencias`, and `page` exclusively via `useSearchParams` from react-router-dom. The `areaAfectada` filter SHALL be a `<select>` populated from `AREAS_SHAC`. Date inputs SHALL be type `date` with `lang="es-PE"`. A "Limpiar filtros" button SHALL appear only when at least one filter param (excluding `page`) is present, and clicking it SHALL remove all filter params and reset `page` to 1. Changing any filter other than `page` SHALL also reset `page` to 1. The filter container SHALL use the shared `FilterBar` component.
...
- **WHEN** `QEListFilters` renders
- **THEN** the Área Afectada control is a `<select>` whose options include each value from `AREAS_SHAC`
...
The system SHALL render a `Ver` icon button (Eye icon) in the Acciones column for every row regardless of role. An `Editar` icon button SHALL appear only when `getQualityEventPermissions(qe.estado, user.rol, isResponsable).puedeEditarCabecera` returns `true`. Both buttons SHALL have `aria-label` and `title` attributes from i18n keys.
...
#### Scenario: User without edit permission does not see Editar button
- **WHEN** a user with role `AUDITOR_INTERNO` renders `QEList`
- **THEN** no edit button appears in any row's Acciones column

#### Scenario: User with edit permission sees Editar button
- **WHEN** `getQualityEventPermissions` returns `puedeEditarCabecera: true` for the current user and QE
- **THEN** an edit icon button appears in that row's Acciones column

---

### Requirement: useQEList maps URL search params to useQualityEvents query
The system SHALL export a `useQEList` hook from `src/features/quality-events/hooks/useQEList.ts` that reads `estado`, `tipo`, `severidad`, `origen`, `areaAfectada`, `fechaDesde`, `fechaHasta`, `soloReincidencias`, and `page` from `useSearchParams`, maps them to a `QEListParams` object with `pageSize: 10`, and delegates to `useQualityEvents()`. The hook SHALL return `{ qualityEvents, isLoading, isError, pagination, refetch }` with no UI logic or JSX.
