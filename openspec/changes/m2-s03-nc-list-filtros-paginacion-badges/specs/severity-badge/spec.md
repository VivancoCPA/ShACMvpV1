...
Componente pill genérico para los 4 niveles de severidad SHAC (BAJA / MEDIA / ALTA / CRITICA). Diseñado para ser reutilizable en M2 (NCs), M3 (Incidentes SyST) y M4 (Quality Events) sin adaptador.
...
The system SHALL use translation keys from the `common` namespace (not `nonconformities`) so that M3 and M4 can use the same component and translation without duplication. Keys SHALL follow the pattern `common:severity.BAJA`, `common:severity.MEDIA`, `common:severity.ALTA`, `common:severity.CRITICA`.
