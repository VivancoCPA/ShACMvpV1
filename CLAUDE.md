...
**Nota técnica — limitación de MSW con descargas de archivo vía `window.open`:** MSW en modo Service Worker intercepta `fetch`/`XHR`, pero nunca una navegación de nivel superior del navegador (`window.open(url)`, cambiar `location.href`, etc.). Si un botón de descarga hace `window.open(url)` directo sobre un string de URL inventado por un fixture MSW (p.ej. `/mock/originales/:id/...`), el navegador navega de verdad a esa ruta, Vite cae al SPA fallback y React Router termina en el catch-all `NotFoundPage`. Este patrón está prohibido para cualquier botón de descarga sobre datos mock (ver M1 `RN-DOC-009` / `document-pdf-security` spec, requirement "Blob-based download pattern for mock file URLs" — caso ya corregido para `archivoOriginalUrl` y `archivoDistribucionUrl`). El patrón correcto en mock: (1) `fetch`/axios sobre el endpoint (interceptado por MSW), (2) leer la respuesta como `Blob`, (3) `URL.createObjectURL(blob)`, (4) disparar la descarga con un `<a download>` temporal creado y removido del DOM, (5) `URL.revokeObjectURL`. El handler MSW correspondiente debe responder con un `Blob`/recurso binario real (puede ser un placeholder mínimo), no solo un string de URL. Esta conversión a Blob es exclusiva del entorno mockeado — el backend .NET real debe seguir devolviendo URLs firmadas navegables directamente; el frontend real puede volver a `window.open(url)` simple o mantener el patrón Blob por consistencia (decisión pendiente hasta que exista el backend). Mismo tipo de limitación que ya existe para `mockSession.ts` con las cookies de sesión.
